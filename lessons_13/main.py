...
if __name__ == "__main__":
    ...
    user = current_session(User).join(Purchase).join(Product).filte()
    product.price > 50
    ).all()
    for user in users:
        print(user.id, user.email)

    product_name = current_session.query(Product).last()
    user = current_session.query(User).join(Purchase).filter(
        or_(Product.name == product.name, prduct.count >=3)
    ).all()
    for user in users:
        print(user.id, user.email)
